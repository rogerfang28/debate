...
OldEventHandler::OldEventHandler(const debate::UIEvent& evt) : evt_(evt) {}
OldEventHandler::OldEventHandler(debate::UIEvent&& evt) : evt_(move(evt)) {}

void OldEventHandler::handleEvent(const string& user) {
    const string actionId = evt_.action_id();
    DebateModerator moderator; // internally uses DebateDatabaseHandler
    OldEventHandler::Log(cout);
    if (actionId == "submitTopic") {
        ...
        if (!topic.empty()) {
            ...
            cerr << "[OldEventHandler] submitTopic event missing topicInput value\n";
        }

    } else if (actionId == "clearTopics") {
        moderator.handleClearDebates(user);
        cout << "[OldEventHandler] clearTopics action processed.\n";

    } else if (actionId == "goHome") {
        // Handle "goHome" action
        goHome(user);
        cout << "[OldEventHandler] goHome action processed for user: " << user << "\n";
        ...
        if (!topicID.empty()) {
            cout << "[OldEventHandler] Entering debate with ID: " << topicID << "\n";
            enterDebate(user, topicID);
            cout << "[OldEventHandler] enterTopic action processed for user: " << user << "\n";
        } else {
            cerr << "[OldEventHandler] enterTopic event missing topicID\n";
        }

    } else {
        cerr << "[OldEventHandler] Unhandled actionId: " << actionId << "\n";
    }
}

// ---------------- Logging helpers ----------------
void OldEventHandler::Log(ostream& os) const {
    os << "[UIEvent]"
       << " id="      << evt_.event_id()
...
void OldEventHandler::PrintEventValue(ostream& os, const debate::EventValue& v) const {
...
const char* OldEventHandler::EventTypeName(debate::EventType t) const {
    switch (t) {
