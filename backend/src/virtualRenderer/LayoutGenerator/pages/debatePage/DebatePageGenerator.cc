...
ui::Component DebatePageGenerator::FillChallenges(user_engagement::UserEngagement engagement, ui::Component mainLayout){
    ...
    for (const auto& challenge : challengesInfo) {
        ...
        int challengeCreatorId = std::stoi(challenge.at("creator_id"));
        bool userOwnsChallenge = (currentUserId == challengeCreatorId);
        Log::test("Adding challenge with ID: " + challengeId + ", owned by user: " + std::to_string(userOwnsChallenge));
        Log::test("User ID: " + std::to_string(currentUserId) + ", Challenge Creator ID: " + std::to_string(challengeCreatorId));
